...
if ($real_source_type == "war") {
    puppi::deploy {
        "${name}-Remove_existing_WAR":
             ...
             user => "root" , project => "$name" , enable => $enable;
        "${name}-Check_undeploy":
             priority => "36" , command => "checkwardir.sh" , arguments => "-a $deploy_root/$source_filename" ,
             user => "$user" , project => "$name" , enable => $enable;
        "${name}-Check_deploy":
             priority => "45" , command => "checkwardir.sh" , arguments => "-p $deploy_root/$source_filename" ,
             user => "$user" , project => "$name" , enable => $enable;
    }
    ...
    puppi::rollback {
        "${name}-Remove_existing_WAR":
             ...
             user => "root" , project => "$name" , enable => $enable;
        "${name}-Check_undeploy":
             priority => "31" , command => "checkwardir.sh" , arguments => "-a $deploy_root/$source_filename" ,
             user => "$user" , project => "$name" , enable => $enable;
        "${name}-Check_deploy":
             priority => "45" , command => "checkwardir.sh" , arguments => "-p $deploy_root/$source_filename" ,
             user => "$user" , project => "$name" , enable => $enable;
...
if ($backup == "full") or ($backup == "diff") {
    puppi::deploy {
        "${name}-Backup_existing_Files":
             priority => "30" , command => "archive.sh" , arguments => "-b $deploy_root -m $backup -o '$backup_rsync_options' -n $backup_retention" ,
             user => "root" , project => "$name" , enable => $enable;
    }
